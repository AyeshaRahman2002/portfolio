export type Mode = 'engineer' | 'artist';
...
export interface EngineeringProject {
  ...
  slug: string;
  year?: string;
  category: 'Research Project' | 'Machine Learning Project' | 'Computer Vision / 3D' | 'Multi-Agent Systems' | 'Time-Series Research';
  purpose: string;
  overview: string;
  problem: string;
  approach: string;
...
export interface Artwork {
  ...
  title?: string;
  year?: string;
  medium: 'Charcoal' | 'Acrylic' | 'Mixed' | string;
  dimensions?: string;
  category?: string;
  description?: string;
  image?: string;
  detailImages?: string[];
  processImages?: ProcessStage[];
  featured?: boolean;
  accent?: string;
  normalMap?: string;
  roughnessMap?: string;
...
export interface SketchbookItem {
  id: string;
  image: string;
  type?: 'study' | 'process' | 'material' | 'note';
  caption?: string;
  year?: string;
}

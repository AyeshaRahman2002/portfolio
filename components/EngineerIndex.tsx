  ...
  return <div className="engineer-index">
    ...
    <section className="project-switcher">
      ...
      <div className="project-switcher__body">
        <nav aria-label="Select a featured project">{projects.map((item,index) => <button key={item.id} type="button" aria-pressed={selected === index} onClick={() => setSelected(index)}><span>{item.number}</span><b>{item.title}</b></button>)}</nav>
    ...
    <section className="engineer-cta"><p>Interested in trustworthy AI, cybersecurity or production ML?</p><a href={`mailto:${SITE_CONFIG.email}`}>START A CONVERSATION ↗</a></section>
  </div>;
}
